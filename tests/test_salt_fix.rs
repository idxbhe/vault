...
fn test_salt_preservation() {
    ...
    // Create and save
    let vault_file1 = VaultFile::new(&vault1, &password, None)
        .expect("Failed to create vault");
    let salt1 = vault_file1.encrypted_payload.salt;
    vault_file1.write(&test_path).expect("Failed to write vault");
    
    println!("✓ Created vault with salt: {:?}", &salt1[..8]);
    
    // Step 2: Read back and get key + salt
    let vault_file2 = VaultFile::read(&test_path)
        .expect("Failed to read vault");
    let salt2 = vault_file2.encrypted_payload.salt;
    let (vault2, key) = vault_file2.decrypt_with_key(&password, None)
    ...
    let vault_file3 = VaultFile::new_with_key(vault2.clone(), &key, &salt2)
        .expect("Failed to create with key");
    let salt3 = vault_file3.encrypted_payload.salt;
    vault_file3.write(&test_path).expect("Failed to write updated vault");
    ...
    let vault_file4 = VaultFile::read(&test_path)
        .expect("Failed to read after save");
